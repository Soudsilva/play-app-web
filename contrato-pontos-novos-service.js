...
const META_ROOT = 'contratos_sociedade/metas/pontos_novos';
const RESUMO_ROOT = 'resumo_pontos_novos_mensal';

export async function carregarResumoPontosNovosMensal(competencia) {
    const chave = String(competencia || '').trim();
    if (!/^\d{4}-\d{2}$/.test(chave)) throw new Error('Competência mensal inválida.');
    const snapshot = await get(ref(db, `${RESUMO_ROOT}/${chave}/quantidadePontos`));
    if (!snapshot.exists()) return { disponivel: false, quantidadePontos: 0 };
    const quantidadePontos = Number(snapshot.val());
    return {
        disponivel: Number.isInteger(quantidadePontos) && quantidadePontos >= 0,
        quantidadePontos: Number.isInteger(quantidadePontos) && quantidadePontos >= 0
            ? quantidadePontos
            : 0
    };
}

export async function criarSolicitacaoMetaPontosNovos(dados) {
    const propostaRef = ref(db, `${META_ROOT}/pendente`);
    const agora = Date.now();
    const proposta = {
        tipo: 'editar_meta_pontos_novos',
        ...
        quantidadeAnterior: Number(dados?.quantidadeAnterior || 0),
        quantidadeNova: Number(dados?.quantidadeNova || 0),
...
export async function responderSolicitacaoMetaPontosNovos(socioIdValor, decisao) {
    ...
    if (proposta.status === 'aprovada') {
        atualizacoes[`${META_ROOT}/ativo`] = {
            quantidade: Number(proposta.quantidadeNova),
...
export async function descartarSolicitacaoMetaPontosNovosExpirada(expiraEmEsperado) {
    const propostaRef = ref(db, `${META_ROOT}/pendente`);
